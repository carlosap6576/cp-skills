...
SYNTHESIS_FILE="/tmp/stock-eval-synthesis-${CLAUDE_SESSION_ID}.md"
...
HTML_PATH="${STOCK_EVAL_MEMORY_DIR:-$SKILL_ROOT/outputs}/${SLUG}-brief.html"
"${STOCK_EVAL_PYTHON}" "${SKILL_ROOT}/scripts/stock_eval.py" "${TOPIC}" \
  --emit=html \
...
- The badge (`🌐 stock-eval vX.Y.Z · synced YYYY-MM-DD`) at the top
...
If the user runs `/stock-eval NVDA` normally, sees the synthesis in chat, and THEN says "save that as HTML" or "give me a shareable version" in a follow-up turn, do the same save flow on the synthesis you wrote in the previous turn. Do not re-research; the synthesis is already in the conversation history. Just write it to the temp file and call the engine with `--emit=html --synthesis-file`.
...
- Do NOT change the file path convention. `${STOCK_EVAL_MEMORY_DIR}/${SLUG}-brief.html` is the canonical location.
...
- **No `${STOCK_EVAL_MEMORY_DIR}` set**: defaults to the skill's `outputs/` folder (`$SKILL_ROOT/outputs`) per the SKILL.md `Configuration` section.
