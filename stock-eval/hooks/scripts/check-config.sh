...
# Check stock-eval configuration status and show appropriate welcome message.
# Priority: .claude/stock-eval.env > ~/.config/stock-eval/.env > env vars

PROJECT_ENV=".claude/stock-eval.env"
GLOBAL_ENV="$HOME/.config/stock-eval/.env"
...
check_perms() {
  ...
  if [[ -n "$perms" && "$perms" != "600" && "$perms" != "400" ]]; then
    echo "/stock-eval: WARNING — $file has permissions $perms (should be 600)."
...
if [[ "${STOCK_EVAL_CONFIG_DIR+x}" == "x" ]]; then
  if [[ -n "$STOCK_EVAL_CONFIG_DIR" ]]; then
    LAST_RUN_FILE="$STOCK_EVAL_CONFIG_DIR/last-run.json"
  else
    LAST_RUN_FILE=""
  fi
else
  LAST_RUN_FILE="$HOME/.config/stock-eval/last-run.json"
fi
LAST_RUN_LINE=""
...
if [[ -z "$SETUP_COMPLETE" && -z "$CONFIG_FILE" && -z "${OPENAI_API_KEY:-}" && -z "${SCRAPECREATORS_API_KEY:-}" && -z "${AUTH_TOKEN:-}" && -z "${XAI_API_KEY:-}" ]]; then
  cat <<'EOF'
/stock-eval: Ready to use. Run /stock-eval to get started — setup takes 30 seconds.
...
if [[ -n "$HAS_SCRAPECREATORS" ]]; then
  ...
  echo "/stock-eval: Ready — ${SOURCE_COUNT} sources active."
  ...
  echo "  Research any topic across social + market + web sources (last 30 days)."
  [[ -n "$LAST_RUN_LINE" ]] && echo "$LAST_RUN_LINE"
  echo "  Tip: set SCRAPECREATORS_API_KEY to add TikTok + Instagram (and a Reddit fallback)."
fi
