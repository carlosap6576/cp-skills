#!/usr/bin/env python3
"""Topic watchlist management for stock-eval."""
...
def _format_delivery_message(topic: str, counts: dict, mode: str) -> str:
    ...
    if mode == "announce":
        return f"📰 *stock-eval update: {topic}*\n{new} new, {updated} updated"
    elif mode == "silent":
        return f"stock-eval: {new} new findings for '{topic}'"
    else:
        return f"stock-eval: Research complete for '{topic}'"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Manage the stock-eval watchlist")
