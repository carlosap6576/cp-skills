#!/usr/bin/env python3
"""Run the v3 verification bundle for stock-eval."""
...
ENGINE = SKILL_ROOT / "scripts" / "stock_eval.py"
        ...
        [
            ...
            *subprocess.run(
                [
                    "rg",
                    "--files",
                    "skills/stock-eval/scripts",
                    "tests",
                    "-g",
                    "*.py",
                    "-g",
                    "!skills/stock-eval/scripts/lib/vendor/**",
                ],
                cwd=REPO_ROOT,
                text=True,
                capture_output=True,
...
def verify_smoke() -> list[dict[str, object]]:
    ...
    for provider, extra in SMOKE_CASES:
        env = os.environ.copy()
        env["STOCK_EVAL_REASONING_PROVIDER"] = provider
        start = time.time()
        result = run_command(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-dir", default="/tmp/stock-eval-v3-verify")
