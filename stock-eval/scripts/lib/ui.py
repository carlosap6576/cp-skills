"""Terminal UI utilities for stock-eval skill."""
...
MINI_BANNER = f"""{Colors.PURPLE}{Colors.BOLD}/stock-eval{Colors.RESET} {Colors.DIM}· researching...{Colors.RESET}"""
...
PROMO_SINGLE_KEY = {
    "reddit": "\n💡 Set SCRAPECREATORS_API_KEY to also pull TikTok and Instagram.\n",
...
To fix this:
1. Add AUTH_TOKEN and CT0 to ~/.config/stock-eval/.env or .claude/stock-eval.env
...
class ProgressDisplay:
    ...
    def _show_banner(self):
        if IS_TTY:
            ...
            sys.stderr.write(f"/stock-eval · researching: {self.topic}\n")
        sys.stderr.flush()
...
def show_diagnostic_banner(diag: dict):
    ...
    if IS_TTY:
        ...
        _header = f"/stock-eval v{_skill_version()} - Source Status"
        ...
        lines.append(f"{Colors.DIM}│{Colors.RESET}                                                     {Colors.DIM}│{Colors.RESET}")
        lines.append(f"{Colors.DIM}│{Colors.RESET}  Config: {Colors.BOLD}~/.config/stock-eval/.env{Colors.RESET}                  {Colors.DIM}│{Colors.RESET}")
        lines.append(f"{Colors.DIM}└─────────────────────────────────────────────────────┘{Colors.RESET}")
    else:
        # Plain text for non-TTY (Claude Code / Codex)
        lines.append("┌─────────────────────────────────────────────────────┐")
        _header_plain = f"/stock-eval v{_skill_version()} - Source Status"
        ...
        lines.append("│                                                     │")
        lines.append("│  Config: ~/.config/stock-eval/.env                  │")
        lines.append("└─────────────────────────────────────────────────────┘")
