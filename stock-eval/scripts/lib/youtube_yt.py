...
def is_ytdlp_installed() -> bool:
    ...
    When STOCK_EVAL_YOUTUBE_SSH_HOST is set, returns True without a local check —
...
def _ytdlp_ssh_host() -> Optional[str]:
    ...
    Set STOCK_EVAL_YOUTUBE_SSH_HOST=<ssh-alias> (e.g. 'macmini') in the environment
    ...
    To use a value from ~/.config/stock-eval/.env, export it into the
    environment before invoking the engine, e.g. in a wrapper:
        set -a; source ~/.config/stock-eval/.env; set +a
        python3 stock_eval.py "..."
    """
    host = os.environ.get("STOCK_EVAL_YOUTUBE_SSH_HOST", "").strip()
    if not host:
        return None
    if not _SSH_HOST_ALIAS_RE.match(host):
        sys.stderr.write(
            f"[youtube_yt] WARNING: STOCK_EVAL_YOUTUBE_SSH_HOST={host!r} "
...
def _wrap_ytdlp_cmd(cmd: List[str]) -> List[str]:
    ...
    STOCK_EVAL_YOUTUBE_SSH_HOST were ever set to a value starting with `-`.
    """
