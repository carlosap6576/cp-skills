"""HTML rendering for shareable stock-eval reports."""
...
CANONICAL_BOUNDARY_PATTERN = re.compile(r"\n?---\n# END OF stock-eval CANONICAL OUTPUT.*$", re.DOTALL)
...
<title>stock-eval · __TITLE__</title>
...
def _markdown_to_html(md: str) -> str:
    ...
    while index < len(lines):
        ...
        if stripped.startswith("🌐 stock-eval"):
            flush_paragraph()
            close_list()
...
def _protect_engine_footers(md: str) -> tuple[str, dict[str, str]]:
    ...
    def replace(match: re.Match[str]) -> str:
        token = f"__STOCK_EVAL_ENGINE_FOOTER_{len(footers)}__"
...
def _wrap_engine_footer(body: str) -> str:
    ...
    return re.sub(
        r"__STOCK_EVAL_ENGINE_FOOTER_\d+__",
        replace,
        body,
    )
...
def _build_colophon(report: schema.Report, *, topic: str | None = None) -> str:
    ...
    rerun = html.escape(f"/stock-eval {display_topic}")
    return (
        '<div class="colophon">\n'
        f"  Generated {generated} by /stock-eval v{html.escape(version)} · topic: {escaped_topic}<br>\n"
