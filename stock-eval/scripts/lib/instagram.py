"""Instagram Reels search via ScrapeCreators API for /stock-eval.
...
# so the default is generous; override via STOCK_EVAL_TRANSCRIPT_TIMEOUT.
...
def _resolve_transcript_timeout(
    ...
    Priority (highest wins):
      ...
      2. ``STOCK_EVAL_TRANSCRIPT_TIMEOUT`` in os.environ
      3. ``STOCK_EVAL_TRANSCRIPT_TIMEOUT`` in caller-supplied config dict
      4. ``DEFAULT_TRANSCRIPT_TIMEOUT`` (30s)

    Mirrors the ``os.environ.get(X) or config.get(X)`` pattern used for
    STOCK_EVAL_STORE in stock_eval.py so the env var works whether it's
    shell-exported or set in ~/.config/stock-eval/.env.
    """
    if timeout is not None:
        try:
            ...
            pass
    raw = os.environ.get("STOCK_EVAL_TRANSCRIPT_TIMEOUT")
    if not raw and config:
        raw = config.get("STOCK_EVAL_TRANSCRIPT_TIMEOUT")
    if raw:
        try:
            return float(raw)
        except (TypeError, ValueError):
...
def fetch_captions(
    ...
    Args:
        ...
        timeout: Optional per-request transcript timeout in seconds. When
            None, resolves from STOCK_EVAL_TRANSCRIPT_TIMEOUT (env or
            config), defaulting to DEFAULT_TRANSCRIPT_TIMEOUT (30s).
        config: Optional config dict (from env.get_config()) used as a
            fallback source for STOCK_EVAL_TRANSCRIPT_TIMEOUT when the
