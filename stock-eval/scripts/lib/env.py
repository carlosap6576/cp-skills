"""Environment and API key management for stock-eval skill."""
...
# Set STOCK_EVAL_CONFIG_DIR="" for clean/no-config mode
# Set STOCK_EVAL_CONFIG_DIR="/path/to/dir" for custom config location
_config_override = os.environ.get('STOCK_EVAL_CONFIG_DIR')
if _config_override == "":
    ...
    CONFIG_DIR = Path.home() / ".config" / "stock-eval"
    CONFIG_FILE = CONFIG_DIR / ".env"
...
# Example: `security add-generic-password -a "$USER" -s stock-eval-XAI_API_KEY -w "xai-..."`.
KEYCHAIN_SERVICE_PREFIX = "stock-eval-"
...
def _check_file_permissions(path: Path) -> None:
    ...
    try:
        ...
        if mode & 0o044:
            sys.stderr.write(
                f"[stock-eval] WARNING: {path} is readable by other users. "
                f"Run: chmod 600 {path}\n"
            )
            sys.stderr.flush()
    except OSError as exc:
        sys.stderr.write(f"[stock-eval] WARNING: could not stat {path}: {exc}\n")
        sys.stderr.flush()
...
def _decode_jwt_payload(token: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        sys.stderr.write(f"[stock-eval] WARNING: malformed JWT token: {exc}\n")
        sys.stderr.flush()
        return None
...
def load_codex_auth(path: Path = CODEX_AUTH_FILE) -> dict[str, Any]:
    ...
    try:
        ...
        sys.stderr.write(
            f"[stock-eval] WARNING: {path} exists but contains invalid JSON -- ignoring\n"
        )
        sys.stderr.flush()
        return {}
...
def _find_project_env() -> Path | None:
    ...
    Searches for .claude/stock-eval.env in each parent directory,
    stopping at the user's home directory or filesystem root.
    """
    cwd = Path.cwd()
    for parent in [cwd, *cwd.parents]:
        candidate = parent / '.claude' / 'stock-eval.env'
        if candidate.exists():
            return candidate
...
def get_config() -> dict[str, Any]:
    ...
    Priority (highest wins):
      ...
      2. .claude/stock-eval.env (per-project config)
      3. ~/.config/stock-eval/.env (global config)
      4. macOS Keychain items prefixed ``stock-eval-`` (Darwin only)
    """
    ...
    keys = [
        ...
        ('STOCK_EVAL_REASONING_PROVIDER', 'auto'),
        ('STOCK_EVAL_PLANNER_MODEL', None),
        ('STOCK_EVAL_RERANK_MODEL', None),
        ('STOCK_EVAL_X_MODEL', None),
        ('STOCK_EVAL_X_BACKEND', None),
        ('STOCK_EVAL_STORE', None),
        ...
        ('INCLUDE_SOURCES', ''),
        ('EXCLUDE_SOURCES', ''),
        ('STOCK_EVAL_YOUTUBE_SSH_HOST', None),
        ('STOCK_EVAL_TRANSCRIPT_TIMEOUT', None),
    ]
...
def get_x_source(config: dict[str, Any]) -> str | None:
    ...
    preferred = (config.get('STOCK_EVAL_X_BACKEND') or '').lower()
...
def is_xiaohongshu_available(config: dict[str, Any]) -> bool:
    ...
    try:
        ...
        sys.stderr.write(
            f"[stock-eval] WARNING: unexpected error checking Xiaohongshu: "
