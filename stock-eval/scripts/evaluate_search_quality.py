#!/usr/bin/env python3
"""Compare two stock-eval revisions on the v3 ranked candidate output."""
...
def run_stock_eval(repo_dir: Path, topic: str, *, search: str, timeout_seconds: int, quick: bool, mock: bool, env: dict[str, str]) -> dict[str, Any]:
    engine = repo_dir / "skills" / "stock-eval" / "scripts" / "stock_eval.py"
    if not engine.exists():
        engine = repo_dir / "scripts" / "stock_eval.py"
...
def create_worktree(rev: str) -> Path:
    worktree_dir = Path(tempfile.mkdtemp(prefix="stock-eval-eval-"))
    subprocess.run(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compare two stock-eval revisions on ranked candidate quality")
...
def main() -> int:
    ...
    try:
        ...
        for topic, query_type in topics:
            try:
                baseline_report = run_stock_eval(
                    baseline_dir,
                    topic,
                    search=args.search,
                ...
                candidate_report = run_stock_eval(
                    candidate_dir,
                    topic,
                    search=args.search,
                    timeout_seconds=args.timeout,
