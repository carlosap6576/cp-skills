#!/bin/bash
# Store stock-eval API keys in the macOS Keychain.
#
# Keys are stored as generic passwords with service name `stock-eval-<KEY>`
...
#   ./setup-keychain.sh --list       # list which stock-eval-* items exist
...
case "$ACTION" in
  list)
    echo "Stored stock-eval-* keychain items:"
