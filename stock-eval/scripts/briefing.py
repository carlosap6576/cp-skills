#!/usr/bin/env python3
"""Morning briefing generator for stock-eval.
...
BRIEFS_DIR = Path.home() / ".local" / "share" / "stock-eval" / "briefs"
...
def generate_daily(since: str = None) -> dict:
    ...
    if not topics:
        return {
            "status": "no_topics",
            "message": "No watchlist topics yet. Add one with: stock-eval watch add \"your topic\"",
        }
...
def main():
    parser = argparse.ArgumentParser(description="Generate stock-eval briefings")
