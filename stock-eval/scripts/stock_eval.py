...
def ensure_supported_python(version_info: tuple[int, int, int] | object | None = None) -> None:
    ...
    if (major, minor) >= MIN_PYTHON:
        return
    sys.stderr.write(
        "stock-eval v3 requires Python 3.12+.\n"
...
def read_synthesis_file(path: str) -> str:
    try:
        ...
        sys.stderr.write(f"[stock-eval] Cannot read --synthesis-file: {exc}\n")
        raise SystemExit(2)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="N",
        help="Auto-discover N competitor entities and fan out stock-eval across all of them as a comparison (default N=2 → 3-way: original + 2 peers; range 1..6). Use --competitors-list to override discovery.",
    )
    parser.add_argument(
        "--competitors-list",
...
def main() -> int:
    ...
    if config.get("STOCK_EVAL_YOUTUBE_SSH_HOST") and "STOCK_EVAL_YOUTUBE_SSH_HOST" not in os.environ:
        os.environ["STOCK_EVAL_YOUTUBE_SSH_HOST"] = config["STOCK_EVAL_YOUTUBE_SSH_HOST"]
    ...
    if args.synthesis_file:
        if args.emit == "html":
            ...
            sys.stderr.write("[stock-eval] Warning: --synthesis-file is only used with --emit=html; ignoring.\n")

    if not os.environ.get("STOCK_EVAL_SKIP_PREFLIGHT"):
            ...
            if comp_explicit:
                ...
                if not resolve_mod._has_backend(config) and not args.mock:
                    sys.stderr.write(
                        ...
                        "  3. Re-invoke: /stock-eval '{topic} vs {peer1} vs {peer2}' "
    ...
    # STOCK_EVAL_STORE env var = persistence default-on. Read both os.environ
    # (for shell-exported users) and config (for users who set it in
    # ~/.config/stock-eval/.env, which env.py loads but does not propagate
    # to os.environ). Mirrors the STOCK_EVAL_DEBUG / STOCK_EVAL_SKIP_PREFLIGHT
    # convention; env-var or config wins, with `--store` flag still working.
    _store_env = (
        os.environ.get("STOCK_EVAL_STORE")
        or config.get("STOCK_EVAL_STORE")
        or ""
    ).lower()
    if args.store or _store_env in ("1", "true", "yes"):
        counts = persist_report(report)
        sys.stderr.write(
            f"[stock-eval] Stored {counts['new']} new, {counts['updated']} updated findings\n"
        )
        sys.stderr.flush()
    ...
    if args.save_dir:
        ...
        sys.stderr.write(f"[stock-eval] Saved output to {save_path}\n")
        ...
        if entity_reports and len(entity_reports) > 1:
            for label, entity_report in entity_reports[1:]:
                ...
                sys.stderr.write(f"[stock-eval] Saved output to {peer_path}\n")
        sys.stderr.flush()
    print(rendered)
    return 0
