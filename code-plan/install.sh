#!/usr/bin/env bash
# code-plan installer & doctor — makes the skill discoverable by Claude
...
#   ./install.sh --dir DIR              # operate on DIR/code-plan only
#   ./install.sh --uninstall            # remove from every candidate root
#   ./install.sh --dir DIR --uninstall  # remove from DIR/code-plan only
set -euo pipefail

SKILL_NAME="code-plan"
