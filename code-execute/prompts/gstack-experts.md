...
Verified against gstack 1.87.0.0 by hand. The **live, machine-verified
...
## `/review` specialist mechanics (gstack 1.64+, verified 1.87.0.0)
...
- **Scope-gated**: `security` (auth scope, or backend scope + >100 lines),
  ...
  `simplification` (diff > 100 lines; advisory only — its rows flag
  unrequested structure and never block).
...
- **Force flags** (put them in the `args` when chaining): `--security`,
  ...
  `--api-contract`, `--design`, `--simplification`, `--all-specialists` — each force-includes
