#!/usr/bin/env bash
# code-execute installer & doctor — makes the skill discoverable by Claude
...
#   ./install.sh --dir DIR              # operate on DIR/code-execute only
#   ./install.sh --uninstall            # remove from every candidate root
#   ./install.sh --dir DIR --uninstall  # remove from DIR/code-execute only
set -euo pipefail
